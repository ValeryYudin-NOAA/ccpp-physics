...
!---------------------------------------------------
      ...
      subroutine cires_ugwp_solv2_v1(mpi_id, master, im, levs, kdt, dtp,    &
...
!  
      ...
      use cires_ugwp_module,only :  krad, kvg, kion, ktg, iPr_ktgw, Pr_kdis, Pr_kvkt
      
      use cires_ugwp_module,only :  knob_ugwp_doheat, knob_ugwp_dokdis, idebug_gwrms
      
      use cires_ugwp_module,only :  psrc => knob_ugwp_palaunch
      
      use cires_ugwp_module,only : maxdudt, maxdtdt, max_eps, dked_min, dked_max
...
!
	   ...
	   zmetk =  azmet(jk)* rhp   
           vueff(jk)  = ktur + kvg(jk)*1. + 2.e-5*exp(zmetk)
...
!   
	    tx1 = sum(abs(dfdz_heat(jk,1:nazd)))/bn2(jk)
