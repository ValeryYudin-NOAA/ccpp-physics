...
!
    ...
    real, parameter    ::  max_kdis = 450.                 ! 400 m2/s
    real, parameter    ::  max_axyz = 450.e-5              ! 400 m/s/day
    real, parameter    ::  max_eps =  max_kdis*4.e-4       ! max_kdis*BN2 
    real, parameter    ::  maxdudt =  max_axyz
    real, parameter    ::  maxdtdt =  max_eps*1.e-3        ! max_kdis*BN2/cp
...
! 
    real, parameter    :: Pr_kvkt = 1./1.                             ! kv/kt = 1./3.    
    real, parameter    :: Pr_kdis = Pr_kvkt/(1.+Pr_kvkt)
...
!   
   real, parameter    :: rain_max=8.e-5, rain_lat=41.0, rain_lim=1.e-5  
   real, parameter    :: w_merra = 1.0,  w_nomerra = 1.-w_merra, w_rain =1.
   real, parameter    :: mtau_rain = 1.e-3, ft_min =0.5, ft_max=2 
   real, parameter    :: tau_ngw_max = 20.e-3                          ! 20 mPa 
   real, parameter    :: tau_ngw_min = .20e-3                          ! .2 mPa    
!   
! Bushell et al. (2015) tau = tau_rainum (~3.8 km) x sqrt(Precip/base_rainum) 
!    
   real, parameter    :: tau_rainum  = 0.7488e-3                       ! 0.74 mPa
   real, parameter    :: base_rainum = 0.1e-5                          ! ~0.1 mm/day  
   real, parameter    :: pbase_um =1./sqrt(base_rainum) * tau_rainum   !  
...
!
    ...
    real,    intent (in) :: con_pi, con_rerth
...
!
!    dxsg =  pi2*arad/float(lonr) * knob_ugwp_ndx4lh ; kxw  =  pi2/dxsg
...
!
       ...
       status = nf90_inquire_dimension(ncid, DimID,  len =ntau_d2t )
           if (me == master)  print *, ntau_d1y, ntau_d2t, ' dimd of taulimb '
	   if (ntau_d2t .le. 0 .or. ntau_d1y .le. 0) then 
	       print *, 'tau-file=',trim(ugwp_taufile)	   
	       print *, 'ntau_d2t=',ntau_d2t, 'ntau_d2t=',ntau_d1y
	       stop
	   endif	   
...
!======================
    ...
    end subroutine cires_ugwp_init_modv1
...
!-----------------------------------------------------------------------
!
!   options for the day-to-day variable sources/spectra and diagnostics
...
!
!   still this option is not active due to "weak" flexibility
...
!
      ...
      integer, dimension(npts) ::  j1_tau,j2_tau
      real ,   dimension(npts) ::  dlat, w1_j1tau, w2_j2tau
...
223   format( 2x, 'vay-limb',  I4, 5(2x, F10.3))
    ...
    subroutine ngwflux_update(me, master, im, levs, kdt, ddd, curdate, &
	    ...
	    xlatd, sinlat,coslat, rain, tau_ngw)
...
!
! GFSv16: cires_ugwp_init_modv0
!    
   subroutine cires_ugwp_init_modv0 (me, master, nlunit, input_nml_file, logunit, &
              fn_nml, lonr, latr, levs, ak, bk, pref, dtp, cdmvgwd, cgwf)
	      
    use ugwp_common,        only :  arad, pi, pi2
    use  ugwp_oro_init,     only :  init_oro_gws
    use  ugwp_conv_init,    only :  init_conv_gws
    use  ugwp_fjet_init,    only :  init_fjet_gws
    use  ugwp_okw_init,     only :  init_okw_gws
...
!
    ilaunch = launch_level
!
! no cotrol of RF settings in GFSphysics namelists only dycore.
!
!    pa_rf   = pa_rf_in
!    tau_rf  = tau_rf_in
...
!
    do k=1, levs
       pmb(k) = ak(k) + pref*bk(k)    ! Pa -unit  Pref = 1.e5
       zkm(k) = -hpskm*alog(pmb(k)/pref)
    enddo
!
! Part-1 :init_global_gwdis
!
!  non-ccpp  call init_global_gwdis(levs, zkm, pmb, kvg, ktg, krad, kion, me,  master)
!
    call init_global_gwdis(levs, zkm, pmb, kvg, ktg, krad, kion, con_pi, me,  master)    
...
! okw
        call init_okw_gws(knob_ugwp_wvspec(4), knob_ugwp_azdir(4), &
                          knob_ugwp_stoch(4), knob_ugwp_effac(4), con_pi,lonr, kxw )
        if (me == master) print *, ' init_okw_gws '
      endif
...
! fronts
        call init_fjet_gws(knob_ugwp_wvspec(3), knob_ugwp_azdir(3), &
                           knob_ugwp_stoch(3), knob_ugwp_effac(3), con_pi,lonr, kxw )
        if (me == master) print *, ' init_fjet_gws '
      endif
...
! conv
        call init_conv_gws(knob_ugwp_wvspec(2), knob_ugwp_azdir(2), &
                    knob_ugwp_stoch(2), knob_ugwp_effac(2), con_pi,con_rerth,lonr, kxw, cgwf )
        if (me == master)   &
           print *, ' init_convective GWs cgwf', knob_ugwp_wvspec(2), knob_ugwp_azdir(2)
...
!
    ...
    module_is_initialized = .true.
    if (me == master)   print *, ' ugwp-V0 is initialized ', module_is_initialized

    end subroutine cires_ugwp_init_modv0
!    
